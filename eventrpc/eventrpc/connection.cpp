...
namespace eventrpc {
int Connection::ConnectionEvent::HandleRead() {
  ASSERT_NE(0, event_flags_ & EVENT_READ);
  return connection_->HandleRead();
}

int Connection::ConnectionEvent::HandleWrite() {
  ASSERT_NE(0, event_flags_ & EVENT_READ);
...
int Connection::HandleRead() {
  ...
  while (true) {
    if (expect_recv_count_ > Connection::BUFFER_LENGTH) {
      recv_count = Connection::BUFFER_LENGTH;
    } else {
